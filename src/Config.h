...
namespace Config {
    struct Options {
        ...
        // ===== Aelove's Tweaks (all disabled by default) =====
        float minStamina = 0.0f;                     // Min stamina to climb (0=disabled)
        bool regularPhysicsOnFall = false;           // Use Havok physics on fall (disables launching)
        bool regularPhysicsOnFallBeast = false;      // Use Havok physics on fall in beast form
        float baseStaminaCostBeast = 0.0f;           // Stamina drain in beast form (0=disabled)
